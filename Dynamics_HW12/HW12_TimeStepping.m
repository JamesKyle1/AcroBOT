...
for i = 1:length(tspan)
    ...
    if sum(currContactMode) ~= sum(oldContactMode)
        disp(['Transition from mode {', num2str(oldContactMode), '} to mode {', num2str(currContactMode), '} at t = ', num2str(tspan(i)), '.']);
    end


    % Reset data
    oldContactMode = currContactMode;
